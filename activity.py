...
class Activity:
    ...
    def __get_display_list(self):
        ...
        for a in self.actions:
            if a.obj.get("名称", None) == "镜头":
                ...
                display_list.append({"index": sys.maxsize, "action": a})
            elif a.obj.get("名称", None) == "更新":
                # 更新角色总是最早执行
                display_list.append({"index": -(sys.maxsize - 1), "action": a})
    ...
    def to_video(self):
        ...
        for display in display_list:
            # action 和 char是在self.__get_display_list()方法里的硬编码，用于区分显示对象的类型
            ...
            if 'char' in display and display["char"].name not in action_chars:
                ...
                for st in self.subtitle:
                    if len(st) > 4 and char.name == st[4]:
                        ...
                        # 如果指定了替换图片就用指定的图片，否则用char自己的图片
                        # 字幕列表的最后一位可能是遍历字幕时添加的图片区间
