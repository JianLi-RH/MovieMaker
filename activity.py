...
class Activity:
    ...
    def __get_timespan(self, obj):
        ...
        # 全部动作的长度
        actions = [Action(self, action) for  action in obj.get("动作", [])] # 不需要总时长
        action_length = 0.0
        calculated_index = []   # save all actions length, for example: {'index': 0, 'length': 5}
        for act in actions:
            ...
            if same_index_action:
                ...
                calculated_index.append({'index': act.render_index, 'length': act.timespan})
    ...
    def to_video(self):
        ...
        for actions in action_list:
            delay_char = len(actions) > 1   # 当同时运行多个动作的时候，需要在所有动作执行结束再绘制其他角色
            ...
            if delay_char:
                # 有bug, 如果角色的图层在动作的角色下面，那么就会被错误的放在动作上面
