...
def zoom_in_out_image(origin_image_path, center, ratio):
    ...
    # dir_name = os.path.dirname(origin_image_path)
    # base_name = os.path.basename(origin_image_path)
    # split_tup = os.path.splitext(base_name)
    # new_path = os.path.join(dir_name, f"{ratio}{split_tup[1]}")
...
def merge_two_image(big_image, small_image, size, pos, rotate=None, overwrite=False):
    ...
    if overwrite:
        ...
        img1 = Image.open(big_image).copy() # 防止覆盖原图
...
if __name__ == "__main__":
    # add_text_to_image("resources/JiChuSuCai/BeiJing/1.jpg", r'中文阿斯asdsad顿萨杜萨的', save_image=True)
