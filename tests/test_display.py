...
class TestDisplayAction(unittest.TestCase):
    def test_display_single_character(self):
        # Given
        ...
        # When
        display.Do(mock_action)

        # Then
        self.assertTrue(mock_char.display)

    def test_display_multiple_characters(self):
        # Given
        mock_action = MagicMock()
        ...
        with patch('actions.display.get_char', side_effect=get_char_side_effect) as mock_get_char:
            # When
            display.Do(mock_action)

            # Then
            self.assertTrue(mock_char1.display)
            self.assertTrue(mock_char2.display)
