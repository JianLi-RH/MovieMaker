...
class Action:
    ...
    def __walk(self, images, sorted_char_list, delay_char: bool):
        ...
        frames = int(len(images) / steps) # 每一个路线需要的帧数
        print("ratio: ", ratio)
        for end_pos in end_pos_list:
            ...
            if isinstance(ratio, list):
                ...
                if not isinstance(ratio, float): # ratio是最终显示比例， 如 0.4
                    try:
                        ratio = float(ratio)
                    except:
                        ratio = 1 # 默认比例不变
            ...
            for i in range(0, frames):
                ...
                rotate = 0
                if mode == "旋转":
                    step_rotate = 360 / self.activity.fps * int(config_reader.round_per_second)  # 每秒旋转圈数
        ...
        for i in range(len(images)):
            # 由于前面使用int填充每个step的frames，所以最后一个step可能数量不足
            # 会导致最后几帧没有动作
            # 所以使用最后一个动作进行填充
    ...
    def __init__(self, activity, obj, timespan=0):
        ...
        if self.obj.get("名称", None) != '更新':
            ...
            self.char = None
        self.render_index = self.obj.get("渲染顺序", 0)    # 动作执行的顺序，数值一样的同时执行， 从小到达执行
